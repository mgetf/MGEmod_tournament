...
ConVar
    ...
    gcvar_spawnFile,
    gcvar_mgemeConnectGrace,
    gcvar_mgemeFragLimit;
...
//mmMatches holds user ids for each arena
char mmMatches[MAXARENAS][2][50]; //char[] is a string (array of chars). char[][] is an array of strings. char[x][y][z] is an array size x of an array size y of strings length z.
...
{	
	...
	KeyValues mgeMeKeyValues = new KeyValues("mge_me");
	bool farts = mgeMeKeyValues.ImportFromFile(txtfile);
	if (!farts) {
		PrintToServer("File not found dummy");
	}
	mgeMeKeyValues.Rewind();
	
	mgeMeKeyValues.GetString("host", wsHost, 64);
	mgeMeKeyValues.GetString("port", wsPortStr, 12);
	mgeMeKeyValues.GetString("apiKey", wsApiKey, 256);
	mgeMeKeyValues.GetString("childId", wsChildId, 16);
	mgeMeKeyValues.GetString("selfHost", serverHost, 64);
	...
	if (wsHandle == INVALID_WEBSOCKET_HANDLE)
		wsHandle = Websocket_Open(wsHost, wsPort, wsIncomingCallback, wsErrCallback, wsCloseCallback); //??
...
/* OnPluginEnd()
*
* Called when the plugin is about to be unloaded. Sourcemod claims you don't need to release most handles here but Websocket insists on it.
* -------------------------------------------------------------------------- */
...
public void OnClientPostAdminCheck(int client)
{
	...
	//mgeme
	char sidBuf[256];
	bool succ = GetClientAuthId(client, AuthId_SteamID64, sidBuf, sizeof(sidBuf)); //write client's steam64 to sidBuf
	if (!succ) {
		ThrowError("False getclientauthid. client: %s", client);
	}
...
//(MGEME) Shoutout user Dolly on the sm discord
//This function loops through clients' steamids and returns the steamid for a certain client
int GetPlayerFromSteamID(const char[] wantSId)
{
	if (StrContains(wantSId, "STEAM") > -1 || StrContains(wantSId, "[") > -1)
	{
		ThrowError("SteamID must be in Steam64 format");
	}
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i) && IsClientAuthorized(i)) //Note: isvalidclient is defined in mgemod, isclientauthorized is defined in sourcemod
		{
			char haveSId[32]; //Buffer for reading steamids
			if (GetClientAuthId(i, AuthId_SteamID64, haveSId, sizeof(haveSId))) //Steam64: ex "7777777777777777"
			{
				if (StrEqual(wantSId, haveSId))
				{
					return i;
				}
			}
...
public void handler_ConVarChange(Handle convar, const char[] oldValue, const char[] newValue)
{
    ...
    if (convar == gcvar_mgemeFragLimit)
	MMFRAGLIMIT = StringToInt(newValue);
...
//(MGEME) when this timer is fired, check which players are in the arena like a good fragger.
public Action Timer_WaitForPlayers(Handle timer, int arenaIdx) {
	PrintToServer("Missing one or both players, killing match.");
	char delinquents[2][50]; //There may one or two delinquents, but there cannot be 0, as that would cause the match to proceed (not cancel)
	char goodboy[50]; //There may or may not be a "good boy" but there cannot be 2, as that would cause the match to proceed (not cancel)
	for (int i = 0; i < 2; i++) {
		char who[50]; //steam64 id
		strcopy(who, 50, mmMatches[arenaIdx][i]); //Copy steam64id into who
		int client = GetPlayerFromSteamID(who);
		if (client == -1) {
			strcopy(delinquents[i], 50, who); //Copy who into delinquents array
			PrintToServer("%s baited match", who);
		} else {
			strcopy(goodboy, 50, who); //This player arrived! Yay!
			RemoveFromQueue(client, false, false);
		}
	}
	//TODO: Send list of delinquents to webserver
	wsSendMatchCancel(delinquents, goodboy, arenaIdx);
	return Plugin_Continue;
}
...
///////////////////////////////////////////////////////
//FinishMatch should perform two duties:
//Clear out extra MGEME handles associated with the end of a match that aren't handled by MGEMod
//(Main duty) Send websocket message back to the server with results
//Params: 	winner/loser are IDs, in a forefit the leaver is laways the loser (even if score may be lower)
//		arena_index: arena they were playing in
//		finished: true for finished duel normally, false for forfeit
public void MGEME_FinishMatch(int winner, int loser, int arena_index, bool finished)
{
	//TODO: (IMPORTANT) When a player ragequits, I won't be able to get their steamid from their stale client id, so keep track carefully 
	//in mmMatches of both players and get IDs from there
	
	//clear data
	mmMatches[arena_index][0] = "";
	mmMatches[arena_index][1] = "";
	
	char winnerSid[50], loserSid[50];
...
public void wsConnCallback(WebSocket sock, any data)
{
	...
	JSONObject payload = new JSONObject();
	payload.SetString("apiKey", wsApiKey);
	payload.SetString("serverNum", wsChildId);
	payload.SetString("serverHost", serverHost);
	payload.SetString("serverPort", serverPort);
	payload.SetString("stvPort", stvPort);
...
public void wsSendMatchCancel(char delinquents[2][50], char goodboy[50], int arenaIdx)
{
	JSONObject msg = new JSONObject();
	msg.SetString("type", "MatchCancel");
	JSONObject payload = new JSONObject();
	JSONArray dels = new JSONArray();
	for (int i = 0; i < sizeof(delinquents); i++) {
		dels.PushString(delinquents[i]);
	}
	payload.Set("delinquents", dels);
	payload.SetString("arrived", goodboy);
	payload.SetInt("arena", arenaIdx);
	bool ok = msg.Set("payload", payload);
	if (!ok)
		PrintToServer("Failed to set json payload");
	char send[1000];
	msg.ToString(send, sizeof(send));
	ws.WriteString(send);
	PrintToServer("Sending mc signal %s", send);
	delete dels;
	delete payload;
	delete msg;
}
...
public void wsSendMatchBegan(char players[2][50])
{
	...
	JSONObject payload = new JSONObject();
	payload.SetString("p1", players[0]);
	payload.SetString("p2", players[1]);
	msg.Set("payload", payload);
	char send[1000];
	msg.ToString(send, sizeof(send));
	ws.WriteString(send);
